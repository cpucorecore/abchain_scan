...
type Unpacker interface {
	Unpack(method string, data []byte, length int) (values map[string]interface{}, err error)
}
...
func (u *unpacker) Unpack(method string, data []byte, length int) (map[string]interface{}, error) {
	values := make(map[string]interface{})
	for _, abi_ := range u.abis {
		err := abi_.UnpackIntoMap(values, method, data)
